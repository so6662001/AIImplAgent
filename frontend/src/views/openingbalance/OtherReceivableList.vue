<template>
  <div>
    ...
    <el-dialog v-model="showDialog" title="新增其他应收" width="700px" destroy-on-close>
...
import { createOtherReceivable, listOtherReceivables } from '@/api/otherreceivable'
import type { OtherReceivable } from '@/types'
import { required } from '@/utils/validators'

const list = ref<OtherReceivable[]>([])
...
async function loadData() {
  ...
  const res = await listOtherReceivables(filterProjectId.value)
...
async function handleSubmit() {
  ...
  try {
    await createOtherReceivable(form.value as unknown as Record<string, unknown>)
    ElMessage.success('其他应收创建成功')
